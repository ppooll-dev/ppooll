...
var vol_sel = 0;
var tpp;
var meter, mix, act_0, act_1, chan_0, chan_1, ramp, pfl, chans, chans_in, xb, info, lcd, pan, volR, volL;
...
function size_obj()
{
	...
	pfl.rect = [lw*4,h1,w/2,h];
	chans.rect = [lw*10,h1,lw*11,h2];
	chans_in.rect = [lw*9,h1,lw*10,h2];
	xb.rect = [w-lw,0,w,h1];
	info.rect = [w-lw,h1,w,h2];
	lcd.rect = [w-lw,0,w+1,h2+2];
...
function draw_lcd()
{
	...
	if(w<300){
		...
		a.message("write", "i");
		if (mix){
			a.message("paintrect", 1,13,lw,26, 6, 6, 153);
			a.message("linesegment", 1,13,1,26, 84, 136, 245);
			} else {
			a.message("paintrect", 1,13,lw,26, 84, 136, 245);
		}
		a.message("linesegment", 0,13,30,13, 6, 6, 153);
		a.message("frgb", 255, 255, 255);
		a.message("moveto", lw/2-2, 22);
		a.message("write", "x");

		} else {
			a.message("moveto", lw/2-1, 23);
			a.message("write", "i");
			
			if (mix){
				a.message("paintrect", 1,0,lw,13, 6, 6, 153);
				a.message("linesegment", 1,0,1,13, 84, 136, 245);
			} else {
				a.message("paintrect", 1,0,lw,13, 84, 136, 245);
			}
			a.message("linesegment", 0,13,30,13, 6, 6, 153);
			a.message("moveto", lw/2-1, 10);
			a.message("frgb", 255, 255, 255);
...
function state(s)
{
	...
	if (jstate[0] != ch){
		...
		script_signals(ch,chcount);
		}
	if (jstate[4] != chcount){
		chcount = jstate[4];
		chans_in.message("set", chcount);
		script_signals(ch,chcount);
		}
	if (jstate[1] != mix){	
		mix = jstate [1];
...
function script_signals(b,c)
{
	...
	if (c==b){
		...
		if (b == 2) chchange = tp.newdefault(99,180,"ll.mc.stereo_pan");
		else {
		if(c>b) {
