{
    "patcher": {
        "fileversion": 1,
        "appversion": {
            "major": 9,
            "minor": 1,
            "revision": 2,
            "architecture": "x64",
            "modernui": 1
        },
        "classnamespace": "box",
        "rect": [ 100.0, 100.0, 643.0, 665.0 ],
        "toolbarvisible": 0,
        "boxes": [
            {
                "box": {
                    "data": {
                        "act": "spat.ambimonitor@",
                        "description": "monitor for ambisonics encoder",
                        "tags": [ "audio converter", "multichannel (mc)" ],
                        "authors": [ "filip", "c. hausch" ]
                    },
                    "hidden": 1,
                    "id": "obj-12",
                    "maxclass": "newobj",
                    "numinlets": 2,
                    "numoutlets": 5,
                    "outlettype": [ "dictionary", "", "", "", "" ],
                    "patching_rect": [ 472.0, 49.0, 61.0, 22.0 ],
                    "saved_object_attributes": {
                        "embed": 1,
                        "legacy": 0,
                        "parameter_enable": 0,
                        "parameter_mappable": 0
                    },
                    "text": "dict",
                    "varname": "for_act_overview"
                }
            },
            {
                "box": {
                    "id": "obj-7",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 48.0, 624.0, 409.0, 20.0 ],
                    "text": "use spat.ambicontrol@ to automate movement of the points ."
                }
            },
            {
                "box": {
                    "id": "obj-8",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 74.0, 567.0, 89.0, 20.0 ],
                    "text": "double click vv"
                }
            },
            {
                "box": {
                    "fontname": "Verdana",
                    "fontsize": 10.0,
                    "id": "obj-6",
                    "maxclass": "newobj",
                    "numinlets": 0,
                    "numoutlets": 0,
                    "patcher": {
                        "fileversion": 1,
                        "appversion": {
                            "major": 9,
                            "minor": 1,
                            "revision": 2,
                            "architecture": "x64",
                            "modernui": 1
                        },
                        "classnamespace": "box",
                        "rect": [ 215.0, 115.0, 382.0, 462.0 ],
                        "toolbarvisible": 0,
                        "enablehscroll": 0,
                        "enablevscroll": 0,
                        "boxes": [
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-7",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 426.0, 64.0, 19.0 ],
                                    "text": "+ ctrl (PC)"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-50",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 85.0, 412.0, 65.0, 19.0 ],
                                    "text": "orientation"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-51",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 412.0, 76.0, 19.0 ],
                                    "text": "+ cmd (Mac)"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-48",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 85.0, 392.0, 140.0, 19.0 ],
                                    "text": "rotation/radial movement"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-49",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 392.0, 36.0, 19.0 ],
                                    "text": "+ alt"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-46",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 85.0, 372.0, 83.0, 19.0 ],
                                    "text": "translation x/z"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-47",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 372.0, 45.0, 19.0 ],
                                    "text": "+ shift"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-44",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 214.0, 59.0, 19.0 ],
                                    "text": "S + shift"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-45",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 195.0, 21.0, 19.0 ],
                                    "text": "S"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-38",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 174.0, 61.0, 19.0 ],
                                    "text": "O + shift"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-39",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 174.0, 261.0, 19.0 ],
                                    "text": "enable/disable orientation (toggle synchronously)"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-40",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 155.0, 21.0, 19.0 ],
                                    "text": "O"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-41",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 155.0, 245.0, 19.0 ],
                                    "text": "enable/disable orientation (toggle individually)"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-36",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 134.0, 59.0, 19.0 ],
                                    "text": "L + shift"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-37",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 134.0, 234.0, 19.0 ],
                                    "text": "lock/unlock selection (toggle synchronously)"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-17",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 115.0, 21.0, 19.0 ],
                                    "text": "L"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-33",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 115.0, 219.0, 19.0 ],
                                    "text": "lock/unlock selection (toggle individually)"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-1",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 85.0, 296.0, 66.0, 19.0 ],
                                    "text": "reset zoom"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-2",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 296.0, 19.0, 19.0 ],
                                    "text": "="
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-3",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 85.0, 276.0, 57.0, 19.0 ],
                                    "text": "zoom out"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-4",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 85.0, 256.0, 50.0, 19.0 ],
                                    "text": "zoom in"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-5",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 276.0, 19.0, 19.0 ],
                                    "text": "-"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-6",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 256.0, 19.0, 19.0 ],
                                    "text": "+"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-12",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 75.0, 21.0, 19.0 ],
                                    "text": "A"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-13",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 95.0, 21.0, 19.0 ],
                                    "text": "D"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-14",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 75.0, 55.0, 19.0 ],
                                    "text": "select all"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-15",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 214.0, 85.0, 19.0 ],
                                    "text": "store snapshot"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-20",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 85.0, 352.0, 84.0, 19.0 ],
                                    "text": "translation x/y"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-21",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 85.0, 235.0, 120.0, 19.0 ],
                                    "text": "delete selected points"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-22",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 331.0, 212.0, 19.0 ],
                                    "text": "nudge selected points (arrow keys)"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-23",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 95.0, 67.0, 19.0 ],
                                    "text": "deselect all"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-24",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 195.0, 86.0, 19.0 ],
                                    "text": "recall snapshot"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-25",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 86.0, 55.0, 119.0, 19.0 ],
                                    "text": "select/deselect points"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-26",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 235.0, 71.0, 19.0 ],
                                    "text": "delete key"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-27",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 352.0, 19.0, 19.0 ],
                                    "text": " "
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana Bold",
                                    "fontsize": 10.0,
                                    "id": "obj-28",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 55.0, 37.0, 19.0 ],
                                    "text": "1 - 9"
                                }
                            },
                            {
                                "box": {
                                    "fontname": "Verdana",
                                    "fontsize": 10.0,
                                    "id": "obj-29",
                                    "maxclass": "comment",
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 16.0, 329.0, 19.0 ],
                                    "text": "(only when the monitor has keyboard-focus, i.e. is highlighted)"
                                }
                            },
                            {
                                "box": {
                                    "angle": 0.0,
                                    "background": 1,
                                    "bgcolor": [ 0.941176, 0.941176, 0.941176, 1.0 ],
                                    "id": "obj-53",
                                    "maxclass": "panel",
                                    "mode": 0,
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 16.0, 411.0, 349.0, 33.0 ],
                                    "proportion": 0.39,
                                    "rounded": 0,
                                    "varname": "arg.2.bg[6]"
                                }
                            },
                            {
                                "box": {
                                    "angle": 0.0,
                                    "background": 1,
                                    "bgcolor": [ 0.941176, 0.941176, 0.941176, 1.0 ],
                                    "id": "obj-52",
                                    "maxclass": "panel",
                                    "mode": 0,
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 16.0, 371.0, 350.0, 20.0 ],
                                    "proportion": 0.39,
                                    "rounded": 0,
                                    "varname": "arg.2.bg[5]"
                                }
                            },
                            {
                                "box": {
                                    "angle": 0.0,
                                    "background": 1,
                                    "bgcolor": [ 0.941176, 0.941176, 0.941176, 1.0 ],
                                    "id": "obj-43",
                                    "maxclass": "panel",
                                    "mode": 0,
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 194.0, 350.0, 40.0 ],
                                    "proportion": 0.39,
                                    "rounded": 0,
                                    "varname": "arg.2.bg[4]"
                                }
                            },
                            {
                                "box": {
                                    "angle": 0.0,
                                    "background": 1,
                                    "bgcolor": [ 0.941176, 0.941176, 0.941176, 1.0 ],
                                    "id": "obj-35",
                                    "maxclass": "panel",
                                    "mode": 0,
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 115.0, 350.0, 40.0 ],
                                    "proportion": 0.39,
                                    "rounded": 0,
                                    "varname": "arg.2.bg[2]"
                                }
                            },
                            {
                                "box": {
                                    "angle": 0.0,
                                    "background": 1,
                                    "bgcolor": [ 0.941176, 0.941176, 0.941176, 1.0 ],
                                    "id": "obj-16",
                                    "maxclass": "panel",
                                    "mode": 0,
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 74.0, 350.0, 20.0 ],
                                    "proportion": 0.39,
                                    "rounded": 0,
                                    "varname": "arg.2.bg[3]"
                                }
                            },
                            {
                                "box": {
                                    "angle": 0.0,
                                    "background": 1,
                                    "bgcolor": [ 0.941176, 0.941176, 0.941176, 1.0 ],
                                    "id": "obj-30",
                                    "maxclass": "panel",
                                    "mode": 0,
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 255.0, 350.0, 20.0 ],
                                    "proportion": 0.39,
                                    "rounded": 0,
                                    "varname": "arg.2.bg[1]"
                                }
                            },
                            {
                                "box": {
                                    "angle": 0.0,
                                    "background": 1,
                                    "bgcolor": [ 0.941176, 0.941176, 0.941176, 1.0 ],
                                    "id": "obj-34",
                                    "maxclass": "panel",
                                    "mode": 0,
                                    "numinlets": 1,
                                    "numoutlets": 0,
                                    "patching_rect": [ 15.0, 294.0, 350.0, 20.0 ],
                                    "proportion": 0.39,
                                    "rounded": 0,
                                    "varname": "arg.2.bg[9]"
                                }
                            }
                        ],
                        "lines": [],
                        "bgcolor": [ 0.98, 0.98, 0.98, 1.0 ]
                    },
                    "patching_rect": [ 48.0, 583.0, 156.0, 21.0 ],
                    "saved_object_attributes": {
                        "locked_bgcolor": [ 0.98, 0.98, 0.98, 1.0 ]
                    },
                    "text": "p Keyboard_Commands"
                }
            },
            {
                "box": {
                    "hidden": 1,
                    "id": "obj-3",
                    "maxclass": "newobj",
                    "numinlets": 1,
                    "numoutlets": 1,
                    "outlettype": [ "" ],
                    "patching_rect": [ 510.75, 349.0, 72.0, 22.0 ],
                    "text": "loadmess 1"
                }
            },
            {
                "box": {
                    "bgcolor": [ 1.0, 1.0, 1.0, 0.0 ],
                    "bgoncolor": [ 0.54902, 0.54902, 0.54902, 0.0 ],
                    "fontsize": 9.0,
                    "id": "obj-70",
                    "legacytextcolor": 1,
                    "maxclass": "textbutton",
                    "numinlets": 1,
                    "numoutlets": 3,
                    "outlettype": [ "", "", "int" ],
                    "parameter_enable": 0,
                    "patching_rect": [ 554.0, 43.0, 77.0, 19.0 ],
                    "text": "www.icst.net",
                    "textcolor": [ 0.0, 0.145098, 0.717647, 1.0 ],
                    "textoncolor": [ 1.0, 0.0, 0.0, 1.0 ],
                    "textovercolor": [ 0.85098, 0.0, 0.0, 1.0 ],
                    "underline": 1,
                    "usebgoncolor": 1,
                    "usetextovercolor": 1
                }
            },
            {
                "box": {
                    "id": "obj-16",
                    "linecount": 2,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 119.0, 508.0, 150.0, 33.0 ],
                    "text": "the zoom and position of the working space."
                }
            },
            {
                "box": {
                    "id": "obj-13",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 98.0, 484.0, 150.0, 20.0 ],
                    "text": "the size of the patch."
                }
            },
            {
                "box": {
                    "id": "obj-11",
                    "linecount": 4,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 98.0, 408.0, 261.0, 60.0 ],
                    "text": "select the spatialisation space:\nhorizontal: will remain in 2 flat dimensions.\nh&v_half: gives half a sphere (elevation 0-90)\nh&v_full: gives a full sphere (elevation -90 - 90)"
                }
            },
            {
                "box": {
                    "id": "obj-25",
                    "linecount": 2,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 93.5, 367.5, 293.0, 33.0 ],
                    "text": "turn the labels for the inputs on/off\nselect \"rename\" to give custome names to the points."
                }
            },
            {
                "box": {
                    "id": "obj-24",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 93.5, 348.5, 108.0, 20.0 ],
                    "text": "turn the grid on/off"
                }
            },
            {
                "box": {
                    "id": "obj-22",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 216.0, 324.5, 263.0, 20.0 ],
                    "text": "click on \"ins\" to focus the mouse to the \"outs\""
                }
            },
            {
                "box": {
                    "id": "obj-21",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 45.0, 305.0, 469.0, 20.0 ],
                    "text": "edit the position of the speakers with the mouse, by hiding the inputs layer temporarily:"
                }
            },
            {
                "box": {
                    "id": "obj-19",
                    "linecount": 2,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 94.0, 268.0, 537.0, 33.0 ],
                    "text": "define the position of each speaker manually with \nazimut (degrees 0-360), elevation (degrees -90 - 90), distance (from center, 0. - 1.)"
                }
            },
            {
                "box": {
                    "id": "obj-18",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 94.0, 246.0, 345.0, 20.0 ],
                    "text": "with default, a standard distribution for the speakers will be set."
                }
            },
            {
                "box": {
                    "id": "obj-17",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 44.0, 202.0, 273.0, 20.0 ],
                    "text": "this creates red points (ins) and white dots (outs)."
                }
            },
            {
                "box": {
                    "id": "obj-14",
                    "linecount": 3,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 103.0, 153.0, 316.5, 47.0 ],
                    "text": "(similar to vbap) you want to define how many soundsources (inputs) should be handled (minimum 1)\nand how many speakers (outputs) (minimum 2)."
                }
            },
            {
                "box": {
                    "bgcolor": [ 0.992157, 0.290837, 0.176198, 1.0 ],
                    "fontface": 0,
                    "fontsize": 14.0,
                    "format": [ 3 ],
                    "id": "obj-23",
                    "label": [ "sz" ],
                    "maxclass": "ll_number",
                    "min": 133.0,
                    "numinlets": 1,
                    "numoutlets": 1,
                    "outlettype": [ "" ],
                    "parameter_enable": 0,
                    "patching_rect": [ 48.0, 487.0, 45.0, 14.0 ],
                    "slidercolor": [ 1.0, 1.0, 1.0, 1.0 ],
                    "slidercolornofocus": [ 0.65098, 0.666667, 0.662745, 0.54902 ],
                    "sliderlog": 5.0,
                    "slidermax": 700.0,
                    "slidermin": 133.0,
                    "textcolor": [ 1.0, 1.0, 1.0, 1.0 ],
                    "textcolornofocus": [ 0.862745, 0.870588, 0.878431, 0.788235 ],
                    "varname": "size",
                    "vertical": -1.0
                }
            },
            {
                "box": {
                    "amount": 4,
                    "fontface": 0,
                    "fontsize": 10.0,
                    "format": [ 1.2 ],
                    "id": "obj-47",
                    "label": [ "zoom", "cx", "cy", "cz" ],
                    "maxclass": "ll_number",
                    "mousefocus": 1,
                    "numinlets": 1,
                    "numoutlets": 1,
                    "outlettype": [ "" ],
                    "parameter_enable": 0,
                    "patching_rect": [ 48.0, 508.0, 62.0, 45.0 ],
                    "slidermax": 3.0,
                    "varname": "zoom"
                }
            },
            {
                "box": {
                    "bgcolor": [ 0.75, 0.75, 0.75, 1.0 ],
                    "bgoncolor": [ 0.741176, 0.184314, 0.756863, 1.0 ],
                    "fontsize": 10.0,
                    "id": "obj-15",
                    "legacytextcolor": 1,
                    "maxclass": "textbutton",
                    "mode": 1,
                    "numinlets": 1,
                    "numoutlets": 3,
                    "outlettype": [ "", "", "int" ],
                    "outputmode": 0,
                    "parameter_enable": 0,
                    "patching_rect": [ 53.5, 353.5, 34.0, 13.0 ],
                    "prototypename": "jit_on",
                    "text": "grid",
                    "textcolor": [ 0.752941, 0.023529, 0.247059, 1.0 ],
                    "texton": "grid",
                    "textoncolor": [ 0.870588, 0.819608, 0.239216, 1.0 ],
                    "textovercolor": [ 0.752941, 0.023529, 0.247059, 1.0 ],
                    "usebgoncolor": 1,
                    "usetextovercolor": 1,
                    "varname": "grid"
                }
            },
            {
                "box": {
                    "bgcolor": [ 0.75, 0.75, 0.75, 1.0 ],
                    "bgoncolor": [ 0.741176, 0.184314, 0.756863, 1.0 ],
                    "fontsize": 10.0,
                    "id": "obj-156",
                    "legacytextcolor": 1,
                    "maxclass": "textbutton",
                    "mode": 1,
                    "numinlets": 1,
                    "numoutlets": 3,
                    "outlettype": [ "", "", "int" ],
                    "outputmode": 0,
                    "parameter_enable": 0,
                    "patching_rect": [ 176.0, 328.0, 34.0, 13.0 ],
                    "prototypename": "jit_on",
                    "text": "outs",
                    "textcolor": [ 0.752941, 0.023529, 0.247059, 1.0 ],
                    "texton": "ins",
                    "textoncolor": [ 0.870588, 0.819608, 0.239216, 1.0 ],
                    "textovercolor": [ 0.752941, 0.023529, 0.247059, 1.0 ],
                    "usebgoncolor": 1,
                    "usetextovercolor": 1,
                    "varname": "edit_in_out"
                }
            },
            {
                "box": {
                    "bgcolor": [ 0.75, 0.75, 0.75, 1.0 ],
                    "bgoncolor": [ 0.741176, 0.184314, 0.756863, 1.0 ],
                    "fontsize": 10.0,
                    "id": "obj-52",
                    "legacytextcolor": 1,
                    "maxclass": "textbutton",
                    "mode": 1,
                    "numinlets": 1,
                    "numoutlets": 3,
                    "outlettype": [ "", "", "int" ],
                    "outputmode": 0,
                    "parameter_enable": 0,
                    "patching_rect": [ 54.5, 371.5, 34.0, 13.0 ],
                    "prototypename": "jit_on",
                    "text": "labels",
                    "textcolor": [ 0.752941, 0.023529, 0.247059, 1.0 ],
                    "texton": "labels",
                    "textoncolor": [ 0.870588, 0.819608, 0.239216, 1.0 ],
                    "textovercolor": [ 0.752941, 0.023529, 0.247059, 1.0 ],
                    "usebgoncolor": 1,
                    "usetextovercolor": 1,
                    "varname": "labels"
                }
            },
            {
                "box": {
                    "bgcolor": [ 0.607843, 0.643137, 0.823529, 1.0 ],
                    "bgoncolor": [ 0.55, 0.55, 0.55, 1.0 ],
                    "id": "obj-96",
                    "legacytextcolor": 1,
                    "maxclass": "textbutton",
                    "numinlets": 1,
                    "numoutlets": 3,
                    "outlettype": [ "", "", "int" ],
                    "outputmode": 0,
                    "parameter_enable": 0,
                    "patching_rect": [ 45.0, 268.0, 44.5, 15.0 ],
                    "prototypename": "vst_button",
                    "text": "define",
                    "textcolor": [ 0.0, 0.0, 0.0, 1.0 ],
                    "textoncolor": [ 1.0, 1.0, 1.0, 1.0 ],
                    "textovercolor": [ 0.0, 0.0, 0.0, 1.0 ],
                    "usebgoncolor": 1,
                    "usetextovercolor": 1,
                    "varname": "define"
                }
            },
            {
                "box": {
                    "bgcolor": [ 0.607843, 0.643137, 0.823529, 1.0 ],
                    "bgoncolor": [ 0.55, 0.55, 0.55, 1.0 ],
                    "id": "obj-1",
                    "legacytextcolor": 1,
                    "maxclass": "textbutton",
                    "numinlets": 1,
                    "numoutlets": 3,
                    "outlettype": [ "", "", "int" ],
                    "outputmode": 0,
                    "parameter_enable": 0,
                    "patching_rect": [ 45.0, 251.0, 44.5, 15.0 ],
                    "prototypename": "vst_button",
                    "text": "default",
                    "textcolor": [ 0.0, 0.0, 0.0, 1.0 ],
                    "textoncolor": [ 1.0, 1.0, 1.0, 1.0 ],
                    "textovercolor": [ 0.0, 0.0, 0.0, 1.0 ],
                    "usebgoncolor": 1,
                    "usetextovercolor": 1,
                    "varname": "default"
                }
            },
            {
                "box": {
                    "fontname": "Arial",
                    "fontsize": 9.0,
                    "id": "obj-98",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 65.0, 157.0, 28.0, 17.0 ],
                    "text": "outs",
                    "textcolor": [ 1.0, 1.0, 1.0, 1.0 ],
                    "varname": "output_label"
                }
            },
            {
                "box": {
                    "fontname": "Arial",
                    "fontsize": 9.0,
                    "id": "obj-99",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 45.0, 157.0, 21.0, 17.0 ],
                    "text": "ins",
                    "textcolor": [ 1.0, 1.0, 1.0, 1.0 ],
                    "varname": "input_label"
                }
            },
            {
                "box": {
                    "bgcolor": [ 0.780392, 0.262745, 0.101961, 1.0 ],
                    "fontface": 1,
                    "fontname": "Arial",
                    "fontsize": 11.0,
                    "htricolor": [ 1.0, 0.929412, 0.133333, 1.0 ],
                    "id": "obj-9",
                    "maxclass": "number",
                    "minimum": 1,
                    "mousefilter": 1,
                    "numinlets": 1,
                    "numoutlets": 2,
                    "outlettype": [ "", "bang" ],
                    "parameter_enable": 0,
                    "patching_rect": [ 44.0, 168.0, 29.0, 21.0 ],
                    "textcolor": [ 0.0, 0.0, 0.0, 1.0 ],
                    "tricolor": [ 0.780392, 0.262745, 0.101961, 1.0 ],
                    "triscale": 0.9,
                    "varname": "inputs"
                }
            },
            {
                "box": {
                    "bgcolor": [ 0.780392, 0.262745, 0.101961, 1.0 ],
                    "fontface": 1,
                    "fontname": "Arial",
                    "fontsize": 11.0,
                    "htricolor": [ 1.0, 0.929412, 0.133333, 1.0 ],
                    "id": "obj-10",
                    "maxclass": "number",
                    "minimum": 2,
                    "mousefilter": 1,
                    "numinlets": 1,
                    "numoutlets": 2,
                    "outlettype": [ "", "bang" ],
                    "parameter_enable": 0,
                    "patching_rect": [ 70.0, 168.0, 29.0, 21.0 ],
                    "textcolor": [ 0.0, 0.0, 0.0, 1.0 ],
                    "tricolor": [ 0.780392, 0.262745, 0.101961, 1.0 ],
                    "triscale": 0.9,
                    "varname": "outputs"
                }
            },
            {
                "box": {
                    "allowdrag": 0,
                    "arrow": 0,
                    "fontname": "Arial Bold",
                    "fontsize": 10.0,
                    "id": "obj-157",
                    "items": [ "horizontal", ",", "h&v_half", ",", "h&v_full" ],
                    "maxclass": "umenu",
                    "numinlets": 1,
                    "numoutlets": 3,
                    "outlettype": [ "int", "", "" ],
                    "parameter_enable": 0,
                    "patching_rect": [ 48.0, 408.0, 45.0, 20.0 ],
                    "varname": "disp_mode"
                }
            },
            {
                "box": {
                    "id": "obj-5",
                    "linecount": 4,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 40.0, 61.0, 409.0, 60.0 ],
                    "text": "it only works together with spat.ambiencoder, spat.ambidecoder\nand  spat.ambipanning.\nso make sure you have one of these open \nand listening to spat.ambimonitor."
                }
            },
            {
                "box": {
                    "id": "obj-4",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 40.0, 37.0, 266.0, 20.0 ],
                    "text": "is a controller for ICST ambisonics spatialisation"
                }
            },
            {
                "box": {
                    "fontsize": 16.0,
                    "id": "obj-2",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 36.0, 6.0, 197.0, 24.0 ],
                    "text": "spat.ambimonitor"
                }
            },
            {
                "box": {
                    "background": 1,
                    "fontname": "Verdana",
                    "fontsize": 9.0,
                    "id": "obj-30",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 489.0, 32.0, 138.0, 17.0 ],
                    "text": "Zurich University of the Arts",
                    "textcolor": [ 1.0, 1.0, 1.0, 1.0 ]
                }
            },
            {
                "box": {
                    "background": 1,
                    "fontname": "Verdana",
                    "fontsize": 9.0,
                    "id": "obj-31",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 416.0, 6.0, 213.0, 17.0 ],
                    "text": "Copyright © 2006 - 2015 by Philippe Kocher",
                    "textcolor": [ 1.0, 1.0, 1.0, 1.0 ]
                }
            },
            {
                "box": {
                    "background": 1,
                    "fontname": "Verdana",
                    "fontsize": 9.0,
                    "id": "obj-32",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 357.0, 19.0, 270.0, 17.0 ],
                    "text": "ICST Institute for Computer Music and Sound Technology",
                    "textcolor": [ 1.0, 1.0, 1.0, 1.0 ]
                }
            }
        ],
        "lines": [
            {
                "patchline": {
                    "destination": [ "obj-15", 0 ],
                    "hidden": 1,
                    "order": 2,
                    "source": [ "obj-3", 0 ]
                }
            },
            {
                "patchline": {
                    "destination": [ "obj-156", 0 ],
                    "hidden": 1,
                    "order": 0,
                    "source": [ "obj-3", 0 ]
                }
            },
            {
                "patchline": {
                    "destination": [ "obj-52", 0 ],
                    "hidden": 1,
                    "order": 1,
                    "source": [ "obj-3", 0 ]
                }
            }
        ],
        "autosave": 0,
        "bgcolor": [ 0.533333, 0.533333, 0.733333, 1.0 ]
    }
}